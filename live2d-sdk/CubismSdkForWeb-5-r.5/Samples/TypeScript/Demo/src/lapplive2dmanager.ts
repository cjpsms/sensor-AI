...
export class LAppLive2DManager {
  ...
  public onUpdate(): void {
    ...
    if (model.getModel()) {
      ...
      // zoom เข้าหน้า+ไหล่ (ปรับ live ได้ผ่าน window._zoom / window._offsetY)
      const zoom = (window as any)._zoom ?? LAppDefine.ModelZoom;
      const offY = (window as any)._offsetY ?? LAppDefine.ModelOffsetY;
      projection.scaleRelative(zoom, zoom);
      projection.translateRelative(0, offY);
    }
  ...
  /**
   * index ที่ระบุไปยังโมเดล (parent page → LAppDelegate → ตรงนี้)
   */
