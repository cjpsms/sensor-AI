...
export class LAppModel extends CubismUserModel {
  ...
  public update(): void {
    ...
    // Mouth control from parent page via postMessage (apply LAST so it wins)
    const mouthVal: number = (window as any)._mouthValue ?? 0;
    const mouthId = CubismFramework.getIdManager().getId('ParamMouthOpenY');
    this._model.setParameterValueById(mouthId, mouthVal);
